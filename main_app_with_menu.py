...
class DatabaseManagementWidget(QWidget):
    ...
    def init_ui(self):
        ...
        # 查看远程用户按钮
        self.view_remote_btn = PrimaryPushButton("查看远程数据库用户")
        self.view_remote_btn.clicked.connect(self.view_remote_users)
        button_layout.addWidget(self.view_remote_btn)
        
        # 查看本地用户按钮
        self.view_local_btn = PushButton("查看本地数据库用户")
        self.view_local_btn.clicked.connect(self.view_local_users)
        button_layout.addWidget(self.view_local_btn)
        ...
        # 结果显示区域
        self.result_text = FluentTextEdit()
        self.result_text.setPlainText("点击上方按钮查看数据库用户信息...")
        layout.addWidget(self.result_text)
        
        self.setLayout(layout)
    
    def view_remote_users(self):
        """查看远程数据库用户"""
        self.result_text.setPlainText("正在查询远程数据库...")
        self.run_user_script("remote")
    
    def view_local_users(self):
        """查看本地数据库用户"""
        self.result_text.setPlainText("正在查询本地数据库...")
        self.run_user_script("local")
    
    def refresh_data(self):
        """刷新数据"""
        self.result_text.setPlainText("数据已刷新，请选择查看远程或本地数据库用户。")
    
    def run_user_script(self, db_type):
        """运行用户查看脚本"""
        try:
            script_path = os.path.join(
                os.path.dirname(__file__), "scripts", "view_users.py"
            )
            result = subprocess.run(
                [sys.executable, script_path, db_type],
                capture_output=True,
                text=True,
...
class SettingsWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.light_theme_btn = PrimaryPushButton("浅色主题")
        self.light_theme_btn.clicked.connect(lambda: setTheme(Theme.LIGHT))
        theme_layout.addWidget(self.light_theme_btn)
        
        self.dark_theme_btn = PushButton("深色主题")
        self.dark_theme_btn.clicked.connect(lambda: setTheme(Theme.DARK))
        theme_layout.addWidget(self.dark_theme_btn)
        
        layout.addLayout(theme_layout)
        
        # 应用信息
        info_text = FluentTextEdit()
