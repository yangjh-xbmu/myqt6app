...
class TestUserServiceIntegration:
    ...
    def testCreateUserSuccess(self):
        """测试成功创建用户"""
        result = self.userService.createUser("newUser", "new@example.com", "password123")
    ...
    def testCreateUserValidation(self):
        ...
        with pytest.raises(ValueError, match="邮箱格式不正确"):
            self.userService.createUser("testUser", "invalid-email", "password123")
        
        # 测试重复用户名
        with pytest.raises(ValueError, match="用户名已存在"):
            self.userService.createUser("admin", "test@example.com", "password123")
    ...
    def testUpdateUser(self):
        """测试更新用户"""
        result = self.userService.updateUser(1, username="newAdmin", email="newadmin@example.com")
    ...
    def testCompleteUserWorkflow(self):
        """测试完整的用户工作流程"""
        # 1. 创建用户
        createResult = self.userService.createUser("workflowUser", "workflow@example.com", "password123")
