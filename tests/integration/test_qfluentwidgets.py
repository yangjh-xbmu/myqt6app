...
from qfluentwidgets import (
    setTheme, Theme, TitleLabel, PrimaryPushButton, 
    PushButton, LineEdit, MessageBox
)
...
class TestQFluentWidgetsIntegration:
    ...
    def testWindowCreation(self, qtApp):
        """测试窗口创建"""
    ...
    @pytest.mark.integration
    def testComponentsExist(self, qtApp):
        """测试组件存在性"""
        window = QFluentWidgetsWindowForTest()
    ...
    @pytest.mark.integration
    def testThemeToggle(self, qtApp):
        """测试主题切换"""
        window = QFluentWidgetsWindowForTest()
    ...
    @pytest.mark.integration
    def testButtonInteraction(self, qtApp, qtBot):
        """测试按钮交互"""
        window = QFluentWidgetsWindowForTest()
        window.show()
