        ...
        if path == 'test' and method == 'GET':
            ...
            try:
                ...
                if role_result['success'] and role_result['data']:
                    try:
                        ...
                        # 尝试从不同位置获取 user_id
                        user_id = None
                        if hasattr(insert_result.get('meta', {}), 'last_row_id'):
                            user_id = insert_result['meta']['last_row_id']
                        elif 'meta' in insert_result and 'last_row_id' in insert_result['meta']:
                            user_id = insert_result['meta']['last_row_id']
                    except Exception:
                    ...
                    if user_id:
                        assign_role_query = """
                            ...
                            VALUES (?, ?)
                        """
                        await execute_db_query(env.DB, assign_role_query, [user_id, role_id])
            ...
            try:
                ...
                if not username_or_email or not password:
                    return Response.new(
                        json.dumps({'error': '用户名/邮箱和密码不能为空'}),
                        {'status': 400, 'headers': cors_headers}
                    )
                
                # 查找用户
                user_query = """
                    SELECT id, username, email, password_hash, salt, display_name, status
                    FROM users 
                ...
                try:
                    ...
                    # 安全地访问用户数据
                    user_salt = user.get('salt') if hasattr(user, 'get') else user['salt']
                    user_hash = user.get('password_hash') if hasattr(user, 'get') else user['password_hash']
                    
                    # 验证密码
                    expected_hash = hash_password(password, user_salt)
                ...
                permissions_result = await execute_db_query(
                    env.DB, permissions_query, [user['id']]
                )
                
                permissions = (permissions_result['data'] 
                              if permissions_result['success'] else [])
